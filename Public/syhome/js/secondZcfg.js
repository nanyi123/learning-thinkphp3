...
function ZcfgAll(myPageNum) {    
    $.ajax({
        url: myUrl+"/syqtsj/index.php/Zcfg/getZcfg?status=2&rows=8&page=" + myPageNum,
        type: "POST",
        dataType: "jsonp",
        error: function () {
...
function showZcfgAll(data) {
   ...
   if (data.total !=0){
    for (var i = 0; i < data.rows.length;i++)
    {
        ...
        myhtml+='<ul><li><a href="../thirdPages/ZcfgDetail.html?id=' + myId + '" target="_blank"'
    ...
    document.getElementById("page").innerHTML = ZcfgPageBar(myAllCount, myPageNum,8);
}
/*通知公告 结束*/

//翻页开始
function ZcfgPageBar(myAllCount, myPageNum, myListCount)
{
    ...
    var myhtml = '<span style="font-size:14px;color:black;">共有[&nbsp;' + myLastPage + '&nbsp;]页,'
          + '每页[&nbsp;' + myListCount + '&nbsp;]条,'
          + '当前第&nbsp;' + myPageNum + '&nbsp;页'
          + '&nbsp;<a href="javascript:ZcfgAll(1)"><span style="font-size:14px;color:black;">首页</span></a>'
          + '&nbsp;<a href="javascript:ZcfgAll(' + myPre + ')"><span style="font-size:14px;color:black;">上一页</span></a>'
          + '&nbsp;<a href="javascript:ZcfgAll(' + myNext + ')"><span style="font-size:14px;color:black;">下一页</span></a>'
          + '&nbsp;<a href="javascript:ZcfgAll(' + myLastPage + ')"><span style="font-size:14px;color:black;">尾页</span></a>'
          + '&nbsp;<a href="javascript:ZcfgGoToPage()"跳转至</span></a>'
          + '<input type="text" id = "txt_GoToPage" class="input" value="1" style="width:30px"/>'
    return myhtml;
}

function ZcfgGoToPage() {
    var myPageNum = parseInt(document.getElementById("txt_GoToPage").value);
    ZcfgAll(myPageNum);
}

function ZcfgDetail(myId) {
    $.ajax({
        url: myUrl+"/syqtsj/index.php/Zcfg/getZcfg?type=Zcfg&status=2&id=" + myId,
        type: "POST",
        dataType: "jsonp",
        error: function () {
            alert("服务器加载错误");
        ...
        success: function (data) {
            showZcfgDetail(data, myId);
        }
    });
}
function showZcfgDetail(data, myID) {
    var myHtml = '<div>';
    ...
    myHtml += '<div>' + myTitle + '</div>'
           + '<div>' + myContent + '</div>'
    myHtml += '</div>';
    document.getElementById("ZcfgDetail").innerHTML = myHtml;
}
