...
function TzggAll(myPageNum) {
    
    $.ajax({
        url: myUrl+"/syqtsj/index.php/Tzgg/getTzgg?status=2&rows=8&page=" + myPageNum,
        type: "POST",
        dataType: "jsonp",
        error: function () {
...
function showTzggAll(data) {
   ...
   if (data.total !=0){
    for (var i = 0; i < data.rows.length;i++)
    {
        ...
        myhtml+='<ul><li><a href="../thirdPages/TzggDetail.html?id=' + myId + '" target="_blank"'
    ...
    document.getElementById("page").innerHTML = TzggPageBar(myAllCount, myPageNum,8);
}
/*通知公告 结束*/

//翻页开始
function TzggPageBar(myAllCount, myPageNum, myListCount)
{
    ...
    var myhtml = '<span style="font-size:14px;color:black;">共有[&nbsp;' + myLastPage + '&nbsp;]页,'
          + '每页[&nbsp;' + myListCount + '&nbsp;]条,'
          + '当前第&nbsp;' + myPageNum + '&nbsp;页'
          + '&nbsp;<a href="javascript:TzggAll(1)"><span style="font-size:14px;color:black;">首页</span></a>'
          + '&nbsp;<a href="javascript:TzggAll(' + myPre + ')"><span style="font-size:14px;color:black;">上一页</span></a>'
          + '&nbsp;<a href="javascript:TzggAll(' + myNext + ')"><span style="font-size:14px;color:black;">下一页</span></a>'
          + '&nbsp;<a href="javascript:TzggAll(' + myLastPage + ')"><span style="font-size:14px;color:black;">尾页</span></a>'
          + '&nbsp;<a href="javascript:TzggGoToPage()"跳转至</span></a>'
          + '<input type="text" id = "txt_GoToPage" class="input" value="1" style="width:30px"/>'
    return myhtml;
}

function TzggGoToPage() {
    var myPageNum = parseInt(document.getElementById("txt_GoToPage").value);
    TzggAll(myPageNum);
}

function TzggDetail(myId) {
    $.ajax({
        url: myUrl+"/syqtsj/index.php/Tzgg/getTzgg?type=tzgg&status=2&id=" + myId,
        type: "POST",
        dataType: "jsonp",
        error: function () {
            alert("服务器加载错误");
...
function showTzggDetail(data) {
    ...
    myHtml += '<div>' + myTitle + '</div>'
           + '<div>' + myContent + '</div>'
    myHtml += '</div>';
    document.getElementById("TzggDetail").innerHTML = myHtml;
}
